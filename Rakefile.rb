...
namespace :build do
  ...
  task :prod do
    ...
    system "JEKYLL_ENV=production bundle exec jekyll build --lsi"
  end
  ...
  task :dev do
    ...
    system "bundle exec jekyll build --config _config.yml,_config.dev.yml --profile --lsi"
  end

  desc "Regenerate files and drafts for development"
  task :drafts do
    puts "* Regenerating files and drafts for development..."
    system "bundle exec jekyll build --config _config.yml,_config.dev.yml --profile --drafts --lsi"
  end
end
...
namespace :notify do
  ...
  task :pingomatic do
    begin
      ...
      XMLRPC::Client.new('rpc.pingomatic.com', '/').call('weblogUpdates.extendedPing', 'nerditya.com' , 'https://nerditya.com', 'https://nerditya.com/atom.xml')
    rescue LoadError
  ...
  task :google do
    begin
      ...
      Net::HTTP.get('www.google.com', '/webmasters/tools/ping?sitemap=' + URI.escape('https://nerditya.com/sitemap.xml'))
    rescue LoadError
  ...
  task :bing do
    begin
      ...
      Net::HTTP.get('www.bing.com', '/webmaster/ping.aspx?siteMap=' + URI.escape('https://nerditya.com/sitemap.xml'))
    rescue LoadError
...
task :rsync do
  ...
  system "rsync --perms --recursive --verbose --compress --delete --chmod=Du=rwx,Dgo=rx,Fu=rw,Fgo=r _site/ root@162.243.149.181:/var/www/nerditya.com/public_html"
end
